...
def tem(liste_variables, message = "") :
    chaine = message + " "
    for v in liste_variables :
        chaine += "'" + v + "' = " + str(v)
    return chaine
...
def incrementCompteur(table, colonne, identifiant, nombre = 1) :
    ...
    if compteur != None :
        ...
        journalErreurs("incrementCompteur ; rien dans " + table + " qui ait comme id. " + identifiant + " ; ou pas de colonne " + colonne + " dans " + table)
    curseur.close()
    branche.close()
...
def choisirCritere(base, egal = False) :   
    if egal :
        ...
        select += tabul(base + 1) + '<input type = "text" name = "valeur" id = "valeur" required/>'
...
def formulaire(action, envoi, colonnes, titre = "", une_ligne = False, renseigne = False, base = 3, creation = False) :
    ...
    for champ in colonnes :
        ...
        ligne += brek + tabul(base + 2) +'<input type="' + typ_input + '" ' + bulle + ' name="' + nom + '" id="' + nom
...
def publication(action, envoi, auteur = "", titre = "", num_blog = -1, base  = 3, id_article = -1) :
    ...
    if id_article > -1 :#modification
        ...
        if article != None :
            ...
            journalErreurs("publication ; rien dans articles qui ait comme id. " + id_article) 
    ...
    l_auteur += tabul(base + 1) + "<input type = 'text' name='auteur' id='auteur' size='50' maxlength = '50' title = '" + title_pseudo + "' value = '" + auteur + "' " + ridonli + " required/><br/>"
    formulaire += l_auteur
    le_titre = tabul(base + 1) + "<label for='titre'>Titre de l'article</label><br/>"
    le_titre += tabul(base + 1) + "<input type = 'text' name='titre' id='titre' size='80' maxlength = '80' title = '" + title_80 + "' value = '" + titre + "' " + ridonli + " required/><br/>"
    formulaire += le_titre
...
def rubrique2(menu) :#huit menus
    ...
    return {"titre" : titre, "qui" : cestQui(), "menu_principal" : baliseMenu(6, 6), "menu" : menu, "contenu" : contenu, "pied_de_page" : baliseMenu(7, 4)}
...
def motDePasseOublie() :
    ...
    return {"titre" : "Attribution d'un nouveau mot de passe", "menu_principal" : baliseMenu(6, 6), "qui" : cestQui(), "menu" : "", "contenu" : contenu, "pied_de_page" : baliseMenu(7, 4)}

@route('/espace_personnel')
@view("temple_hote.tpl")
def espacePersonnel() :
    #pseudo = request.get_cookie("connecte")
    #if pseudo : #le zig est connecté
    corpus = ['déconnexion','modifier mes données', 'publier un article']
    menu = {}
    for mot in corpus : 
        menu[mot] = urliser(mot)
    menu = baliseMenu(8, 3)
    ...
    #else :
        #menu = ""
        #contenu = "Il faut d'abord se connecter pour accéder à son espace personnel."
    return {"titre" : "", "menu_principal" : baliseMenu(6, 6), "qui" : cestQui(), "menu" : menu, "contenu" : contenu, "pied_de_page" : baliseMenu(7, 4)}
...
def connexion() :
    ...
    if request.forms.connexion :
        ...
        if examen(pseudo, mot_de_passe):
            ...
            titre = "Connexion avec le pseudo et le mot de passe (il faut être abonné)"   
    return {"titre" : titre, "menu_principal" : baliseMenu(6, 6), "menu" : "", "qui" : cestQui(), "contenu" : contenu, "pied_de_page" : baliseMenu(7, 4)}
...
def deconnexion() :
    ...
    return {"titre":"Déconnexion","menu_principal":baliseMenu(6, 6),"menu" :"","qui":cestQui(),"contenu":"","pied_de_page":baliseMenu(7, 4)}
...
def edition(tache) :
    ...
    if request.forms.edit :
        ...
        if id_abonne != None :
            ...
            journalErreurs("edition ; pas d'abonné dont l'id. soit " + id_abonne)
        curseur.close()
        branche.close()
    ...
    curseur.close()
    branche.close()
    return {"titre":"Modification de mes données personnelles","menu_principal":baliseMenu(6, 6),"menu" :"","qui":cestQui(),"contenu":contenu,"pied_de_page":baliseMenu(7, 4)}
...
def choix(tache) :
    ...
    if request.forms.modifier :
        ...
        curseur.close()
        branche.close()
        return {"titre" : "Modification d'un article", "menu_principal" : baliseMenu(6, 6), "qui" : cestQui(), "menu" : "", "contenu" : message, "pied_de_page" : baliseMenu(7, 4)} 
        ...
        return {"titre" : "Publication d'un article", "menu_principal" : baliseMenu(6, 6), "qui" : cestQui(), "menu" : "", "contenu" : message, "pied_de_page" : baliseMenu(7, 4)}
        ...
        return {"titre" : "Publication d'un article", "menu_principal" : baliseMenu(6, 6), "qui" : cestQui(), "menu" : "", "contenu" : contenu, "pied_de_page" : baliseMenu(7, 4)} 
    ...
    if verbe == 'modifier' :
        ...
        if pseudo == 'Jean-Max':#premier passage
            ...
            return {"titre" : "Saisie du titre de votre article", "menu_principal" : baliseMenu(6, 6), "menu" : "", "qui" : cestQui(), "contenu" : contenu, "pied_de_page" : baliseMenu(7, 4)}
    ...
    return {"titre" : tititre, "menu_principal" : baliseMenu(6, 6), "qui" : cestQui(), "menu" : "", "contenu" : contenu, "pied_de_page" : baliseMenu(7, 4)}
...
def nouveau() :
    ...
    titre = "<h1>Pour participer à vaste-programme.fr</h1>"
    url = "abonnement.html"
    try :
        c = open("html/" + url)
        texte = c.read()
    ...
    if echec :
        contenu = formulaire(action = "/abonnement", envoi = "envoi_abonnement", colonnes = [("pseudo", "50"), ("mail", "50"), ("mot de passe", "50")], creation = True) + message
    return {"titre" : titre, "qui" : cestQui(), "menu_principal" : baliseMenu(6, 6), "menu" : "", "contenu" : texte + message + contenu, "pied_de_page" : baliseMenu(7, 4)}
...
def contact() :
    ...
    return {"titre" : "Contacter la librairie-école", "qui" : cestQui(), "menu_principal" : baliseMenu(6, 6), "menu" : "", "contenu" : contenu, "pied_de_page" : baliseMenu(7, 4)}    
...
def jeux() :
    ...
    return {"titre" : "Le jeu est plaisant donc utile", "qui" : cestQui(), "menu_principal" : baliseMenu(6, 6), "menu" : "", "contenu" : contenu, "pied_de_page" : baliseMenu(7, 4)}
...
def bibliotheque() :
    ...
    curseur.close()
    branche.close()
    return {"titre" : "Livres petits et grands au format PDF", "qui" : cestQui(), "menu_principal" : baliseMenu(6, 6), "menu" : "", "contenu" : contenu, "pied_de_page" : baliseMenu(7, 4)}    
...
def pageBlog(seize_blogs):
    ...
    return {"titre" : "", "qui" : cestQui(), "menu_principal" : baliseMenu(6, 6), "menu" : baliseMenu(maman, 5), "contenu" : contenu, "pied_de_page" : baliseMenu(7, 4)}
...
def templeHote(page_blog) :
    ...
    branche = sqlite3.connect('vasteprogramme.db')
    curseur = branche.cursor()
    req = "SELECT auteur FROM articles WHERE id = ? "
    curseur.execute(req, (id_article,))
    auteur = curseur.fetchone()[0]
    maman = int(page_blog[sterling + 1 :])
    pseudo = request.get_cookie("connecte")
    bouton_zombie = ""
    if pseudo == auteur :   
    ...
    titre = "<div class = 'ligne_titre'>"
    titre += tabul(base + 1) + "<p>Article, par</p>"
    titre += tabul(base + 1) + "<div class = 'nom_auteur'>" + auteur + "</div>" + bouton_zombie + tabul(base) + "</div>"
    url = radicande + ".html"
    try :
        c = open("html/" + url)
        article = c.read()
    ...
    if note != None :
        ...
        personnes_bottees = ""
        journalErreurs("templeHote ; rien dans articles qui ait comme id. " + id_article) 
    ...
    contenu = article + saisie_samebotte + saisie_commentaire + message + liste
    return {"titre" : titre, "qui" : cestQui(), "menu_principal" : baliseMenu(6, 6), "menu" : baliseMenu(maman, 6), "contenu" : contenu, "pied_de_page" : baliseMenu(7, 4)}
...
run(host='localhost', port=environ.get('PORT'))